...
class FCNNv2(nn.Module):
    def __init__(self, hparams: dict):
        ...
        self._input_size = np.asarray(input_size) if len(
            input_size) == 2 else np.asarray(input_size[1:])
    ...
    def forward(self, x):
        ...
        out = F.interpolate(out, size=tuple(
            (self._input_size / 2).astype(np.uint8)), mode="bilinear", align_corners=True)
        ...
        out = F.interpolate(out, tuple(self._input_size.astype(np.uint8)),
