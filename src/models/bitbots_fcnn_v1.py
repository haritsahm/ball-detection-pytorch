...
from torchmetrics.classification.accuracy import Precision, Recall
...
class FCNNv1LitModel(LightningModule):
    """Lightning model for Bitbots's FCNN v1 and v2.
    """
    ...
    def step(self, batch: Any):
        ...
        loss = self.criterion(logits_x, gtcenter_x) + self.criterion(logits_y, center_y)
        preds = F.softmax(logits_x, dim=1), F.softmax(logits_y, dim=1)

        return loss, preds, gtcenter_x, gtcenter_y, imgInfo
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        # log train metrics
        acc = [func(arg_pred_x, arg_pred_y, arg_gt_x, arg_gt_y) for func in self.train_radacc]

        self.log("train/loss", loss, on_step=False, on_epoch=True, prog_bar=False)
        self.log("train/acc_3", acc[0], on_step=False, on_epoch=True, prog_bar=True)
        self.log("train/acc_5", acc[2], on_step=False, on_epoch=True, prog_bar=True)
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        ...
        acc = [func(arg_pred_x, arg_pred_y, arg_gt_x, arg_gt_y) for func in self.val_radacc]

        # log val metrics
        self.log("val/loss", loss, on_step=False, on_epoch=True, prog_bar=False)
        self.log("val/acc_3", acc[0], on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/acc_5", acc[2], on_step=False, on_epoch=True, prog_bar=True)
    ...
    def validation_epoch_end(self, outputs: List[Any]):
        ...
        self.log("val/acc_rad10_best", self.val_acc_best[2].compute(), on_epoch=True, prog_bar=True)
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        acc = [func(arg_pred_x, arg_pred_y, arg_gt_x, arg_gt_y) for func in self.test_radacc]

        # log test metrics
        self.log("test/loss", loss, on_step=False, on_epoch=True)
        self.log("test/acc_3", acc[0], on_step=False, on_epoch=True, prog_bar=True)
        self.log("test/acc_5", acc[2], on_step=False, on_epoch=True, prog_bar=True)
